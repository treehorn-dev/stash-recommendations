...
Task 8: review findings open - durable outbox uses nullable protocol `event_id` as the mutation key. `source_snapshot` and `hook` rows therefore cannot be acked, retried, or quarantined because SQLite `WHERE event_id = NULL` matches no rows. Fix with a durable non-null row identity, preserve protocol event IDs for idempotency, add SQLite migration coverage, and prove snapshot/hook ack/retry/quarantine behavior with tests.
Task 8: fix round 1/5 (1 addressed, 0 open - durable non-null row identity now drives outbox mutation paths while protocol `event_id` remains optional/idempotent; commits 3392e80..48791da)
Task 8: complete (commits 3392e80..48791da, review clean)
Task 8: review findings open - retry scheduling does not persist the current error into `last_error`, so status diagnostics go stale after transient delivery failures; and `StashClient.find_scene()` assumes `findScene` is always an object, so deleted/missing local scenes raise an incidental `TypeError` instead of a safe not-found result. Fix both with regressions and keep Task 9 capture work out of scope.
Task 8: fix round 2/5 addressed - `record_retry(row_id, now, error)` now persists the current retry error while retaining pending/backoff semantics, and `StashClient.find_scene()` now returns `None` when GraphQL reports `findScene: null`; regressions cover both behaviors. Verification: `PYTHONPATH=plugin/stashRecommendations pytest -q plugin/stashRecommendations/tests/test_outbox.py plugin/stashRecommendations/tests/test_stash_client.py` passed (11 passed), `make test-plugin` passed (64 passed), and `git diff --check` passed.
Task 9: complete (implementation pending independent review). Implemented scope-limited plugin capture/import work only: `capture-rating` now materializes local `rating100` changes into per-StashID set/remove preference events without delivery; `sync-ratings`, `sync-engagement`, and `sync-metadata` now preview counts and enqueue scoped work on confirmation; engagement import tracks only new recorded `play_history` / `o_history` events with persistent client identity and monotonic sequence allocation; and configured Stash-box source reads now rate-limit, map schema-limited scene/performer snapshots, and queue them with `source_updated_at`. Verification: `PYTHONPATH=plugin/stashRecommendations pytest plugin/stashRecommendations/tests/test_stash_client.py plugin/stashRecommendations/tests/test_sync.py -q` passed after a red phase that failed on missing `rating100` capture and non-client-specific history IDs (10 passed green), `PYTHONPATH=plugin/stashRecommendations pytest plugin/stashRecommendations/tests/test_capture.py plugin/stashRecommendations/tests/test_source_client.py plugin/stashRecommendations/tests/test_snapshots.py plugin/stashRecommendations/tests/test_sync.py -q` passed (13 passed), `make test-plugin` passed (76 passed), and `git diff --check` passed.
Task 9: fix round 1/5 (1 addressed, 0 open - configured Stash-box scene fetches now request schema-compatible `groups { group { id name } }`, with source-client and snapshot regressions covering query shape plus retained mapping; commit pending)
Task 9: fix round 2/5 addressed - pending deterministic engagement event IDs now suppress re-enqueue even if the `synced_history_events` marker is missing, preventing duplicate outbox `event_id` crashes; and `queue_metadata_sync()` now isolates per-key fetch/mapping failures, returns explicit `failed`/`errors` details, and continues queueing later configured keys in the same run. Verification: `PYTHONPATH=plugin/stashRecommendations pytest plugin/stashRecommendations/tests/test_sync.py -q` passed after a red phase that failed on duplicate outbox `event_id` and `source_updated_at is required` aborting the sync loop (7 passed green), `PYTHONPATH=plugin/stashRecommendations pytest plugin/stashRecommendations/tests/test_capture.py plugin/stashRecommendations/tests/test_source_client.py plugin/stashRecommendations/tests/test_snapshots.py plugin/stashRecommendations/tests/test_sync.py -q` passed (16 passed), `make test-plugin` passed (79 passed), and `git diff --check` passed. Awaiting scoped review before Task 10.
